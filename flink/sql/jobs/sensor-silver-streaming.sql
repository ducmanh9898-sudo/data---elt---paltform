...
CREATE TEMPORARY TABLE sensor_kafka_silver_source (
    ...
    produced_at_ts AS (
        CASE
            WHEN produced_at_utc IS NULL THEN
                CAST(NULL AS TIMESTAMP_LTZ(3))
            WHEN UNIX_TIMESTAMP(
                produced_at_utc,
                'yyyy-MM-dd''T''HH:mm:ss.SSSX'
...
SELECT
    ...
    CAST(event_time_ts AS TIMESTAMP_LTZ(6)),
    CAST(produced_at_ts AS TIMESTAMP_LTZ(6)),
    ...
    CAST('VALID' AS STRING),
    CAST(NULL AS STRING),

    CAST(CURRENT_TIMESTAMP AS TIMESTAMP_LTZ(6))
...
FROM (
    ...
    WHERE
        ...
        AND produced_at_ts IS NOT NULL
